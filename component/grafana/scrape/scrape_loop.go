...
func newScrapePool(cfg Arguments, appendable grafana.Appendable, logger log.Logger) (*scrapePool, error) {
...
func newScrapeLoop(t *Target, scrapeClient *http.Client, appendable grafana.Appendable, interval, timeout time.Duration, logger log.Logger) *scrapeLoop {
	return &scrapeLoop{
		Target:       t,
		logger:       logger,
		scrapeClient: scrapeClient,
...
func (t *scrapeLoop) scrape() {
	...
	if err := t.fetchDashboards(scrapeCtx, buf); err != nil {
		level.Error(t.logger).Log("msg", "fetch dashboards failed", "target", t.Labels().String(), "err", err)
	...
	if err := t.appender.Append(context.Background(), b); err != nil {
...
func (t *scrapeLoop) fetchDashboards(ctx context.Context, buf io.Writer) error {
	if t.req == nil {
	...
	level.Debug(t.logger).Log("msg", "scraping dashboards", "url", t.req.URL.String())
	...
	if len(b) == 0 {
		return fmt.Errorf("empty dashboards", t.req.URL.String())
	}
